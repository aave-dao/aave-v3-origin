...
import {PercentageMath} from '../munged/src/contracts/protocol/libraries/math/PercentageMath.sol';
...
contract Utilities {
  ...
  function percentMulCeil(uint256 value, uint256 percentage) external pure returns (uint256 result) {
    return PercentageMath.percentMulCeil(value,percentage);
  }

  function percentMulFloor(uint256 value, uint256 percentage) external pure returns (uint256 result) {
    return PercentageMath.percentMulFloor(value,percentage);
  }

  function percentDiv(uint256 value, uint256 percentage) external pure returns (uint256 result) {
    return PercentageMath.percentDiv(value,percentage);
  }

  function percentDivCeil(uint256 value, uint256 percentage) external pure returns (uint256 result) {
    return PercentageMath.percentDivCeil(value,percentage);
  }

  function percentDivFloor(uint256 value, uint256 percentage) external pure returns (uint256 result) {
    return PercentageMath.percentDivFloor(value,percentage);
  }
