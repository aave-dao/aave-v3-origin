...
// Shared rules for AToken, and VariableDebtToken: equivalence checks
...
filtered { f -> f.contract == currentContract && !f.isView && !ignore_func(f) }
{
    ...
    uint _scaled_bal_in_contract = aToken.scaledBalanceOf(owner);
    uint _scaled_bal_in_CVL = scaledBalanceOfCVL(aToken, owner);
    require _scaled_bal_in_CVL == _scaled_bal_in_contract;
