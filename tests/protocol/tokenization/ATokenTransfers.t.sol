...
contract ATokenTransferTests is TestnetProcedures {
  ...
  function test_scaled_balance_token_base_alice_transfer_to_bob_accrues_interests() public {
    ...
    uint256 expectedScaledTransferAmount = transferAmount.rayDivCeil(
      expectedIndex
    );
    ...
    uint256 senderBalanceIncrease = aliceScaledBalanceBefore.rayMulFloor(
      expectedIndex
    ) - aliceScaledBalanceBefore.rayMulFloor(previousIndex);
    uint256 recipientBalanceIncrease = bobScaledBalanceBefore.rayMulFloor(
      expectedIndex
